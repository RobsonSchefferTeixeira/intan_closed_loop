        ...
        public void CheckForUsbData(Queue<USBData> plotQueue, Queue<USBData> saveQueue)
        {
            ...
            if (haveEnoughData)
            {
                ...
                if (synthDataMode)
                {
                    ...
                    float[] buffer = new float[750];
                    float amplitude = 20;
                    float frequency = 8;
                    for (int n = 0; n < buffer.Length; n++)
                    {
                        
                        buffer[n] = (float)(amplitude * Math.Sin(2 * Math.PI * infinitycounter / sampleRate * frequency)) + (float)(5 * Math.Sin(2 * Math.PI * infinitycounter / sampleRate * 60));
                        
                        if(infinitycounter == 25000)
                        {
                            infinitycounter = 0;
                        }
                        else
                    ...
                    for (channel = 0; channel < 16; channel++)
                    {
                        for (i = 0; i < 750; i++)
                        {
                            ...
                            dataRaw[channel, i] = (float)synthSpikeOffset[channel] + 2.0F * gaussian(myRand) + buffer[i];  // create realistic offset and background of 2.0uV rms noise
